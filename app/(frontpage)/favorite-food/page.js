...
function FavoriteFoodPage() {
  ...
  const [selectedMeals, setSelectedMeals] = useState(state.cheatMealFoodItems || []);
...
useEffect(() => {
...
}, [state.isAuthChecked, state.isAuthenticated, state.dietType, state.currentStep, router, updateStep]);
    ...
    try {
      ...
      if (response.success) {
        ...
        const customFoods = selectedMeals
          .filter(selectedMeal => !apiMeals.some(meal => meal.name === selectedMeal))
          .map(customFood => ({
        ...
        const sortedMeals = allMeals.sort((a, b) => b.user_count - a.user_count);
      ...
      showAlert('error', 'Failed to load cheat meals. Please check your internet connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCustomFoodAdd = () => {
    if (customFood.trim()) {
      ...
      if (!selectedMeals.includes(newFood) && !cheatMeals.some(meal => meal.name === newFood)) {
  ...
  const handleContinue = (e) => {
    ...
    if (selectedMeals.length === 0) {
      showAlert('warning', 'Please select at least one favorite food or cheat meal.');
      return;
    }
                ...
                {loading ? (
                  <div className="text-center py-4">
                    <div className="spinner-border text-primary" role="status">
                      <span className="visually-hidden">Loading cheat meals...</span>
                    </div>
                      ...
                      {cheatMeals.map((meal) => (
                        <div key={meal.id} className="food-bx">
                          ...
                          <label 
                            ...
                            className={selectedMeals.includes(meal.name) ? 'selected' : ''}
                          >
                            {meal.name}
