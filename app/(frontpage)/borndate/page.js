...
function BornDatePage() {
  ...
  const handleDateChange = () => {
    if (selectedMonth && selectedDay && selectedYear) {
      ...
      const dateString = `${selectedYear}-${String(monthIndex + 1).padStart(2, '0')}-${String(selectedDay).padStart(2, '0')}`;
      ...
      if (age < 13) {
        showAlert('warning', 'You must be at least 13 years old to use this app.');
        return;
      }
  ...
  const handleContinue = (e) => {
    ...
    if (!selectedMonth || !selectedDay || !selectedYear) {
      showAlert('warning', 'Please select your complete birth date to continue.');
      return;
    }
                ...
                <form onSubmit={handleContinue}>
                  ...
                  <div className="text-center mt-4">
                    <button
                      ...
                      disabled={!selectedMonth || !selectedDay || !selectedYear || state.age < 13}
                    >
                      Continue
                    </button>
                  </div>
